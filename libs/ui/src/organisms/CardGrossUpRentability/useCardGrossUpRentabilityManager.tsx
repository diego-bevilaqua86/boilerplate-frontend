          ...
          <Paper key={item.label + index} withBorder radius="md">
            ...
            <Stack gap="xs" px="md" py="sm">
              <Group justify="space-between">
                <Text size="sm" c="dimmed"><Trans>% Rent. Nominal</Trans></Text>
                <Text size="sm">{percentFormatter(item.nominalReturn, 2)}</Text>
              </Group>
              <Box bg="gray.0" p="xs" style={{ borderRadius: 6 }}>
                <Stack gap={4}>
                  <Group justify="space-between">
                    <Text size="sm" c="dimmed"><Trans>% Rent. c/ Gross Up</Trans></Text>
                    <Text size="sm" fw={600}>{percentFormatter(item.grossUpReturn, 2)}</Text>
                  </Group>
                  <Group justify="space-between">
                    <Text size="sm" c="dimmed"><Trans>Impacto</Trans></Text>
                    <Text size="sm" fw={600}>{percentFormatter(item.grossUpImpact, 2)}</Text>
                  </Group>
                </Stack>
              </Box>
            </Stack>
